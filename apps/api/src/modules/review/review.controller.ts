...
export async function reviewRoutes(app: FastifyInstance) {
  app.get('/api/v1/review-inbox', { preHandler: [authMiddleware] }, async (req) => {
    const query = req.query as { repo_id?: string };
    return reviewService.listInbox(req.auth.orgId, query.repo_id);
  });

  app.post('/api/v1/repos/:repoId/review/:ruleId/resolve', { preHandler: [authMiddleware, requireRole('owner', 'admin')] }, async (req, reply) => {
    ...
    const result = await reviewService.resolve(req.auth.orgId, req.auth.userId, repoId, ruleId, body);
  ...
  app.post('/api/v1/repos/:repoId/review/bulk-resolve', { preHandler: [authMiddleware, requireRole('owner', 'admin')] }, async (req, reply) => {
    ...
    const result = await reviewService.resolveBulk(req.auth.orgId, req.auth.userId, repoId, body);
  ...
  app.post('/api/v1/repos/:repoId/review/:ruleId/assign', { preHandler: [authMiddleware, requireRole('owner', 'admin')] }, async (req, reply) => {
    const { repoId, ruleId } = req.params as { repoId: string; ruleId: string };
    const body = req.body as ReviewAssignRequest;
    if (!body.assigned_to) return reply.status(400).send({ error: 'assigned_to is required' });

    const result = await reviewService.assign(req.auth.orgId, req.auth.userId, repoId, ruleId, body);
  ...
  app.post('/api/v1/repos/:repoId/review/bulk-assign', { preHandler: [authMiddleware, requireRole('owner', 'admin')] }, async (req, reply) => {
    const { repoId } = req.params as { repoId: string };
    const body = req.body as BulkReviewAssignRequest;
    if (!body.assigned_to) return reply.status(400).send({ error: 'assigned_to is required' });
    if (!Array.isArray(body.rule_ids) || body.rule_ids.length === 0) return reply.status(400).send({ error: 'rule_ids is required' });

    const result = await reviewService.assignBulk(req.auth.orgId, req.auth.userId, repoId, body);
