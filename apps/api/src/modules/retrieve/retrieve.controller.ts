...
export async function retrieveRoutes(app: FastifyInstance) {
  app.post('/api/v1/retrieve', { preHandler: [authMiddleware] }, async (req, reply) => {
    const body = req.body as RetrieveRequest;
    if (!body.repo) return reply.status(400).send({ error: 'repo is required' });
    const retrieveRequest: RetrieveRequest = { ...body, user_id: body.user_id ?? req.auth.userId };

    const start = Date.now();
    const result = await retrieveService.retrieve(req.auth.orgId, retrieveRequest);
