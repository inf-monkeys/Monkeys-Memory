...
import { listJsonFiles, padLeft, padRight, pathExists, readConfig, readJson } from "./lib/utils.mjs";
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    if (argv[i] === "--help" || argv[i] === "-h") {
      console.log("Usage: monkeys-memory status [--repo <repo>]");
      process.exit(0);
    }
    if (argv[i] === "--repo") {
...
try {
  ...
  console.error(`[monkeys-memory] status failed: ${error.message}`);
  process.exit(1);
}
