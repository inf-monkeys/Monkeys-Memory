...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    if (argv[i] === "--help" || argv[i] === "-h") {
      console.log("Usage: monkeys-memory deprecate --repo <repo> --id <experience-id>");
      process.exit(0);
    } else if (argv[i] === "--repo") { args.repo = argv[i + 1] ?? null; i += 1; }
    else if (argv[i] === "--id") { args.id = argv[i + 1] ?? null; i += 1; }
  }
  if (!args.repo) { console.error("[monkeys-memory] --repo is required"); process.exit(1); }
  if (!args.id) { console.error("[monkeys-memory] --id is required"); process.exit(1); }
  return args;
}
...
try {
  ...
  if (!found) {
    console.error(`[monkeys-memory] experience "${args.id}" not found in ${args.repo}`);
    process.exit(1);
  }
  ...
  console.error(`[monkeys-memory] deprecate failed: ${error.message}`);
  process.exit(1);
}
